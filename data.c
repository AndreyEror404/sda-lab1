...
void saveCatalogToFile(const Catalog* catalog, const char* filename) {
    ...
    fwrite(&catalog->size, sizeof(int), 1, file);
    fwrite(&catalog->capacity, sizeof(int), 1, file);
    fwrite(catalog->books, sizeof(Book), catalog->size, file);

    fclose(file);
    printf("Каталог сохранен в файл %s\n", filename);
}
...
void loadCatalogFromFile(Catalog* catalog, const char* filename) {
    ...
    if (size > 0) {
        catalog->books = (Book*)malloc(capacity * sizeof(Book));
        catalog->size = size;
        catalog->capacity = capacity;
        fread(catalog->books, sizeof(Book), size, file);
    } else {
        catalog->size = 0;
        catalog->capacity = 0;
        catalog->books = NULL;
    ...
    fclose(file);
    printf("Каталог загружен из файла %s\n", filename);
}